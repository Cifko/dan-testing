...
class DanWalletDaemons(Collection[DanWalletDaemon]):
    ...
    def add(self, indexer_jrpc: int):
        id = len(self.items)
        self.items[id] = DanWalletDaemon(id, indexer_jrpc, signaling_server.json_rpc_port, local_ip)
    ...
    def http(self, index: int) -> Optional[str]:
        ...
        if index in self.items:
            return f"http://{self.items[index].http_ui_address}"
        return None
